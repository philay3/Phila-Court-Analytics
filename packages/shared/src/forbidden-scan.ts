 ...
 *
 * Relocated from apps/api/src/test-support (task 15.2) into @pca/shared so the
 * task-10.1 API privacy suite AND the Sprint-3 rendered-page E2E suite reuse
 * ONE checker over the same shared term/value lists. Exposed via the dedicated
 * `@pca/shared/forbidden-scan` subpath so it stays off the main runtime surface.
 * No behavior change from the 10.1 checker — only the import path moved.
 */
