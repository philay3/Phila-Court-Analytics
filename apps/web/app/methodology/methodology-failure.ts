...
/**
 * Maps a methodology-fetch failure to the user-facing message to render (task
 * 14.2). Mirrors definitions-failure (14.1): both arms resolve to a controlled,
 * copy-safety-scanned @pca/shared constant — never an inline string and never
 * the API's own `message` field or any request detail:
 ...
 */
export function methodologyFailureMessage(failure: PublicApiFailure): string {
